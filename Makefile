...
DEB_NAMES := clearwater-snmp-handler-homestead clearwater-snmp-handler-cdiv clearwater-snmp-alarm-agent clearwater-snmp-handler-memento-as clearwater-snmp-handler-memento clearwater-snmp-handler-astaire

# Add dependencies to deb-only (target will be added by build-infra)
deb-only: cw_alarm_agent homestead_handler.so cdiv_handler.so memento_handler.so memento_as_handler.so astaire_handler.so
